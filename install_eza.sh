...
(apt update >/dev/null 2>&1 && apt install -y gpg >/dev/null 2>&1) &
...
spinner $! "Создаю директорию keyrings..."
if [ $? -ne 0 ]; then
...
spinner $! "Скачиваю и устанавливаю ключ..."
if [ $? -ne 0 ]; then
...
spinner $! "Добавляю репозиторий eza..."
if [ $? -ne 0 ]; then
...
spinner $! "Устанавливаю права на файлы..."
if [ $? -ne 0 ]; then
  echo -e "${RED}Не смог установить права, проверь доступ!${NC}" >&2
  exit 1
fi

(apt update >/dev/null 2>&1 && apt install -y eza >/dev/null 2>&1) &
spinner $! "Установка eza..."
if [ $? -ne 0 ]; then
